...
public class ResourceOrder extends Encoding {
	 ...
	 public ResourceOrder(Instance instance) {
		 super(instance);
	     M = new Task[instance.numMachines][instance.numJobs];
	      
	 }
	 
	 @Override
	    public Schedule toSchedule() {
	        // time at which each machine is going to be free
	        ...
	        int[] nextMachineSpot = new int[instance.numMachines];
	        ...
	        while(modified) {
	            ...
	            for (int job = 0; job < instance.numJobs; job++) {
	                for (int machine = 0; machine < instance.numMachines; machine++) {
	                	...
	                	//au début task vaut 0 pour tout les jobs
	                	int task = nextTask[job];
	                    
	                	//La tâche que l'on va traiter 
	                	Task t1=new Task(job+1,task+1);
	                    
	                	//si il reste encore une tâche à réaliser pour la machine 
	                    if (nextMachineSpot[machine] < instance.numJobs){
	                    	//si la tâche que l'on veut programmer est la même que celle dans la matrice 
	                        if (M[machine][nextMachineSpot[machine]].job==t1.job && M[machine][nextMachineSpot[machine]].task==t1.task) {
	                        	//si c'est la première tâche d'un job 
	                            if (task == 0) {
